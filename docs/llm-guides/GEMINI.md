# NSIP MCP Server -- Gemini Instructions

The NSIP MCP server provides 13 tools for querying the National Sheep Improvement Program database -- animal search, EBV comparison, inbreeding analysis, and mating recommendations -- via Model Context Protocol.
...
Rank current rams in breed 486 by terminal sire traits:
...
This returns the top 5 rams ranked by a weighted composite score. Negative weight on BWT penalizes heavy birth weight (lower is preferred for that trait).
