...
| `CHANGELOG.md` | **Do not hand-edit.** `changelog.yml` regenerates it with git-cliff (`cliff.toml`) on the release tag and opens an auto-merged PR into develop (e.g. `chore/changelog-v<X.Y.Z>` → "docs: update CHANGELOG.md for v<X.Y.Z>"). Your only job is to confirm `## [Unreleased]` has real content from conventional commits — if it's empty, stop and ask what this release contains. The per-release GitHub Release notes are likewise generated by git-cliff inside release.yml. |
